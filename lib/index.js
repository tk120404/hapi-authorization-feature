...
internals.onPreHandler = async function(request, h) {
    ...
    if (params) {
        ...
        try {
            //Role check
            if (params.role || params.function) {
                if (params.role) {
                    ...
                    let index = this.config.functions.indexOf(params.function) // api route definition and the plugin configuration
                    if (index === -1) {
                        throw Boom.forbidden('Unauthorized');
                    }
                }
            ...
            //feature Check
            if (params.validateFeature && (params.feature || params.subfeature)) {
                if (params.feature) {
                    let index = this.config.features.indexOf(params.feature)
                    if (index === -1) {
                        throw Boom.forbidden('Unauthorized');
                    }
                } else if (params.subfeature) {
                    let index = this.config.subfeatures.indexOf(params.subfeature)
                    if (index === -1) {
                        throw Boom.forbidden('Unauthorized');
                    }
                }
