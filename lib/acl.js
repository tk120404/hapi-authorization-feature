...
internals.isGranted = function(user, params) {
    ...
    if (!params.validateFeature || userFeatures && userFeatures.indexOf(params.feature) !== -1 || userSubfeatures && userSubfeatures.indexOf(params.subset) !== -1) {
        if (userRoles && isUserRole || userFunctions && userFunctions.indexOf(params.functionality) !== -1) {
            accessGranted = true;
        }
    }
